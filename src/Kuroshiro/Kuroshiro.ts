...
export type Sillabary = 'hiragana' | 'katakana' | 'romaji'
...
export class Kuroshiro {
  private _analyzer?: KuromojiAnalyzer
  /**
     * Constructor
     * @constructs Kuroshiro
     */
  constructor(analyzer?: KuromojiAnalyzer) {
    this._analyzer = analyzer || new KuromojiAnalyzer()
  }
  ...
  */
  async convert(str: string, options?: ConvertOptions): Promise<string> {
    options = options || {}
    options.to = options.to || 'hiragana'
    options.mode = options.mode || 'normal'
    options.romajiSystem = options.romajiSystem || RomanizationSystem.Hepburn
    ...
    const ROMAJI_SYSTEMS = Object.values(RomanizationSystem)
    if (ROMAJI_SYSTEMS.indexOf(options.romajiSystem) === -1) {
      throw new Error('Invalid Romanization System.')
    }

    const rawTokens: KuromojiParsedResponse[] = await this._analyzer!.parse(str)
    const tokens: KuromojiParsedResponse[] = patchTokens(rawTokens)
